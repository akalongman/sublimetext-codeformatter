...
indent_tags = [%d]
indent_unindent_tags = [%d]
""" % (self.indent_size, self.indent_char, self.indent_with_tabs, self.preserve_newlines, self.max_preserve_newlines, self.indent_tags, self.indent_unindent_tags)
...
class Beautifier:
	...
	def __init__(self, source_text, opts=default_options()):
		...
		self.indentUnindentTags = opts.indent_unindent_tags


	def beautify(self):

			ignored_tag_opening = "<script|<style|<!--|<!---|<cfabort|<cfargument|cfbreak|<cfdump|<cfparam|<cfset|{\\*|<\\?php"
			...
			for item in rawcode_flat_list:
				...
				elif re.search(tag_indent_unindent, item, re.IGNORECASE):
					indent_level = indent_level - 1
					tmp = (indent_char * indent_level) + item
					indent_level = indent_level + 1
